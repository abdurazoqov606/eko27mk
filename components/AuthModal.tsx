...
const AuthModal: React.FC<AuthModalProps> = ({ onClose, onLoginSuccess }) => {
  ...
  const avatars = [
    { seed: 'Abbos', label: 'Quvnoq' },
    { seed: 'Felix', label: 'Aqlli' },
    { seed: 'Aneka', label: 'Jasur' },
    { seed: 'Zoe', label: 'Mehribon' },
    { seed: 'Max', label: 'Chaqqon' },
    { seed: 'Luna', label: 'Guzal' },
    { seed: 'Oliver', label: 'Faol' },
    { seed: 'Maya', label: 'Eko' }
  ];
    ...
    return (
      <div className="fixed inset-0 z-[110] flex items-center justify-center p-4">
        ...
        <div className="relative bg-white w-full max-w-lg rounded-[64px] p-12 text-center border-t-[12px] border-rose-500">
           ...
           <h3 className="text-3xl font-black text-slate-900 mb-4 uppercase italic leading-none">Profil Mavjud</h3>
           <p className="text-slate-500 font-bold mb-10 italic">Xavfsizlik qoidalariga ko'ra, bir qurilmadan faqat bitta profil yaratish mumkin.</p>
      ...
      <div className="relative bg-white w-full max-w-lg rounded-[64px] shadow-3xl overflow-hidden p-10 md:p-14 border-t-[12px] border-emerald-600">
        ...
        <form onSubmit={handleSubmit} className="space-y-8">
           <div className="space-y-2">
              ...
              <input required type="text" placeholder="Masalan: Abbos..." value={formData.name} onChange={e => setFormData({...formData, name: e.target.value})} className="w-full px-8 py-5 bg-slate-50 rounded-[32px] font-black text-lg outline-none border-2 border-transparent focus:border-emerald-500/30 transition-all shadow-inner" />
           </div>

           <div className="space-y-4">
              <label className="text-[10px] font-black text-slate-400 uppercase tracking-[0.3em] block ml-4">Avatar tanlang</label>
              <div className="grid grid-cols-4 gap-4">
                 {avatars.map(av => (
                   <button 
                    key={av.seed} 
                    type="button" 
                    onClick={() => setFormData({...formData, avatarSeed: av.seed})} 
                    className={`aspect-square rounded-[24px] overflow-hidden border-4 transition-all hover:scale-110 active:scale-90 ${
                      formData.avatarSeed === av.seed ? 'border-emerald-500 bg-emerald-50 shadow-xl' : 'border-slate-50 opacity-60 grayscale-[50%]'
                    }`}
                   >
                      <img src={`https://api.dicebear.com/7.x/adventurer/svg?seed=${av.seed}&backgroundColor=b6e3f4,c0aede,d1d4f9`} className="w-full h-full object-cover" alt={av.label} />
                   </button>
                 ))}
              </div>
           </div>

           <button disabled={loading || !formData.name} className="w-full py-6 bg-emerald-600 text-white rounded-[32px] font-black text-xl shadow-2xl flex items-center justify-center gap-4 disabled:opacity-50 hover:bg-emerald-500 transition-all active:scale-95">
