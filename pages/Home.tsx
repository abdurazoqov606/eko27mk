...
import { ArrowRight, Sparkles, Users, Trophy, ShieldCheck, Newspaper, Coins } from 'lucide-react';
...
const Home: React.FC<HomeProps> = ({ onNavigate, onAction }) => {
  const handleFunding = () => {
  ...
  const handleJoin = () => {
    if (onAction) onAction();
    onNavigate(AppSection.COMMUNITY_CHAT);
  };
        ...
        <div className="max-w-7xl mx-auto px-4 relative z-10 py-16 md:py-24 text-center">
          ...
          <div className="mb-8 md:mb-12 animate-slide-up">
            ...
            <div className="flex items-center justify-center gap-3">
              ...
              <p className="text-emerald-400 font-black text-[10px] md:text-xl uppercase tracking-[0.4em] italic">Abdurazoqov Abbos</p>
              <div className="h-px w-8 md:w-12 bg-emerald-500/50" />
            </div>
          </div>
          <div className="flex flex-col sm:flex-row items-center justify-center gap-4 md:gap-6 animate-slide-up delay-200">
            <button onClick={handleJoin} className="w-full sm:w-auto px-8 md:px-12 py-4 md:py-6 bg-emerald-600 text-white rounded-2xl md:rounded-[32px] font-black text-lg md:text-xl hover:bg-emerald-500 transition-all shadow-xl flex items-center justify-center gap-3 group">
              Qo'shiling <ArrowRight size={20} className="group-hover:translate-x-2 transition-transform" />
            </button>
            <button onClick={handleFunding} className="w-full sm:w-auto px-8 md:px-12 py-4 md:py-6 bg-white/10 backdrop-blur-xl text-white border border-white/20 rounded-2xl md:rounded-[32px] font-black text-lg md:text-xl hover:bg-white/20 transition-all flex items-center justify-center gap-3 group">
      ...
      <section className="py-16 md:py-32 bg-white dark:bg-slate-950 rounded-t-[40px] -mt-10 relative z-20">
        <div className="max-w-7xl mx-auto px-4 md:px-6 lg:px-8">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6 md:gap-8">
            <FeatureCard icon={<Users />} title="Eko-Chat" desc="Hamjamiyat bilan muloqot." onClick={() => onNavigate(AppSection.COMMUNITY_CHAT)} color="bg-blue-50 text-blue-600" />
            <FeatureCard icon={<Trophy />} title="Tanlovlar" desc="Yutuqli eko-aksiyalar." onClick={() => onNavigate(AppSection.NEWS_FORUM)} color="bg-amber-50 text-amber-600" />
            <FeatureCard icon={<ShieldCheck />} title="Muammolar" desc="Ekologik vaziyat tahlili." onClick={() => onNavigate(AppSection.PROBLEMS)} color="bg-rose-50 text-rose-600" />
            <FeatureCard icon={<Newspaper />} title="Yangiliklar" desc="So'nggi eko-xabarlar." onClick={() => onNavigate(AppSection.NEWS)} color="bg-emerald-50 text-emerald-600" />
          </div>
        </div>
      </section>
    </div>
...
const FeatureCard = ({ icon, title, desc, onClick, color }: any) => (
  <div onClick={onClick} className="bg-white dark:bg-slate-900 p-8 md:p-10 rounded-[32px] md:rounded-[48px] border border-slate-100 dark:border-white/5 shadow-sm hover:shadow-xl hover:-translate-y-1 transition-all cursor-pointer group">
